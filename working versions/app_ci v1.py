...
# Tabs
tab1, tab2, tab3 = st.tabs(["📁 Upload & Index", "💬 Query & Insights", "⚙️ Program Profile"])
...
with tab1:
    ...
    if input_method == "📝 Paste Text" and 'process_paste' in locals() and process_paste and pasted_text:
        ...
        with st.expander("📄 Pasted Content", expanded=True):
            try:
                ...
                # Auto-index button
                if st.button(f"🚀 Index This Document", key="index_pasted"):
                    with st.spinner("Indexing..."):
                        try:
                            # Generate document ID
                            doc_id = f"doc_{uuid.uuid4().hex[:8]}"
    ...
    try:
        memory = get_memory()
        docs = memory.list_documents(limit=50)
            ...
            for doc in docs:
                with st.expander(f"📄 {doc['filename']} ({doc['detected_type']})"):
                    ...
                    with col2:
                        ...
                        st.markdown(f"**Indexed**: {'✓' if doc['indexed'] else '✗'}")

                    if doc.get('topics'):
                        # Safely parse topics list from JSON string
                        try:
                            if isinstance(doc['topics'], str):
                                topics_list = json.loads(doc['topics'])
                            else:
                                topics_list = doc['topics']
                            st.markdown(f"**Topics**: {', '.join(topics_list)}")
                        except (json.JSONDecodeError, TypeError):
                            # Fallback: display as-is if parsing fails
                            st.markdown(f"**Topics**: {doc['topics']}")
        else:
