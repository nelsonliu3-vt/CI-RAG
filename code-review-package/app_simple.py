...
if program_name:
    ...
    st.success(f"✓ Program: **{program_name}**")
...
st.info(f"💡 **Auto-Analysis**: Will comprehensively analyze all uploaded documents for competitive impact on **{program_name if program_name else '[Your Program]'}**")
...
if analyze_clicked:
    with st.spinner("🔍 Searching documents..."):
        try:
            # Use auto-generated comprehensive query
            sanitizer = get_sanitizer()
            sanitized_query = sanitizer.sanitize_query(auto_query, max_length=2000, strict=False)

            # Hybrid search
            hybrid_search = get_hybrid_search()
            results = hybrid_search.hybrid_search(sanitized_query, top_k=10)

            if not results:
                st.warning("⚠️ No relevant documents found. Try uploading more documents or rephrasing your question.")
            else:
                st.info(f"Found {len(results)} relevant document sections")

                # Reranking
                with st.spinner("📊 Analyzing relevance..."):
                    reranker = get_reranker()
                    results = reranker.rerank(sanitized_query, results, top_k=10)
